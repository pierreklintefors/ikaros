...
class CurrentPositionMapping : public Module
{
    ...
    matrix SetGoalCurrent(matrix present_current, int increment, int limit, matrix position, matrix goal_position, int margin)
    {
        ...
        for (int i = 0; i < current_controlled_servos.size(); i++)
        {
            ...
            // Check if the servo needs more current (below limit AND outside margin)
    ...
    matrix ApproximatingGoal(matrix present_position, matrix previous_position, matrix goal_position, int margin)
    {
        for (int i = 0; i < current_controlled_servos.size(); i++)
        {
            ...
            if (abs(goal_position(servo_indx) - present_position(servo_indx)) < 0.97 * abs(goal_position(servo_indx) - previous_position(servo_indx)))
            {
    ...
    void SaveMetricsToJson(matrix goal_positions, matrix start_position, matrix max_current, matrix min_current, matrix min_torque, matrix overshot, matrix time, matrix ticks, std::string robotType, int transition, long long unique_id)
    {
        std::string scriptPath = __FILE__;
        std::string scriptDirectory = scriptPath.substr(0, scriptPath.find_last_of("/\\"));
        std::string filePath = scriptDirectory + "/data/CurrentPositionMapping" + robotType + "Increment_" + std::to_string((int)current_increment);
        std::string suffix;
        if (ConsistencyTest)
        {
        ...
        std::string directory = scriptDirectory + "/data";
        ...
        if (transition == 0)
        {
            ...
            if (fileExists) {
                fileEmpty = checkFile.peek() == std::ifstream::traits_type::eof();
            }
            checkFile.close();
        ...
        std::ofstream file;
        if (appendToExisting && transition == 0) {
    ...
    void SaveTrajectory(std::vector<std::shared_ptr<std::vector<float>>> &trajectory,
                        ...
                        matrix start_position, matrix goal_positions)
    {
        ...
        else
        {
            // Remove "}\n]\n}" (5 characters) and add a comma
            std::fstream file(filePath, std::ios::in | std::ios::out);
            file.seekp(-4, std::ios::end);  // Position before "}\n]\n}"
            file << ",\n{\n\"UniqueID\":" << unique_id << ",\n";
            file.close();
        }
        ...
        // Include start and goal position data in the JSON
        json << ",\n\"StartPosition\":[";
        for (size_t i = 0; i < current_controlled_servos.size(); i++) {
            int servo_idx = current_controlled_servos[i];
            json << std::fixed << std::setprecision(2) << float(start_position[servo_idx]);
        ...
        for (size_t i = 0; i < current_controlled_servos.size(); i++) {
            int servo_idx = current_controlled_servos[i];
            json << std::fixed << std::setprecision(2) << float(goal_positions[servo_idx]);
            if (i < current_controlled_servos.size() - 1)
                json << ",";
        }
        json << "]";
    ...
    void Init()
    {
        ...
        Bind(torque_enable, "TorqueEnable");
        ...
        torque_enable.set_name("TorqueEnable");
        torque_enable.copy(present_current);
        torque_enable.set(1);
        ...
        Notify(msg_debug, "Generated Unique Run ID: " + std::to_string(unique_id));
