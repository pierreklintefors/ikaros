...
class TestMapping: public Module
{
    ...
    matrix ApproximatingGoal(matrix &present_position,  matrix goal_position, int margin){
        ...
        for (int i =0; i < servos_to_test.size(); i++){
            ...
            if (abs(goal_position(servos_to_test(i)) - present_position(servos_to_test(i))) < 0.97*abs(goal_position(servos_to_test(i))-previous_position(servos_to_test(i)))){
                         ...
                         std::string model_name, matrix model_prediction)
    {
        ...
        for (int i = 0; i < servos_to_test.size(); i++)
        {
            ...
            // <<< EDIT 7: Add NearestNeighbor logic >>>
                 ...
                 if (query_build_ok && present_position.size() > servo_idx && goal_position.size() > servo_idx) {
                     query_point_raw[1] = goal_position(servo_idx) - present_position(servo_idx);
                 ...
                 if (query_build_ok && goal_position.size() > servo_idx) {
                     query_point_raw[2] = goal_position(servo_idx);
                 } else if (query_build_ok) {
    ...
    void Init()
    {
        ...
        Bind(starting_positions, "StartPosition");
        ...
        if(robotType.as_string() == "Torso"){
            servos_to_test.set_name("CurrentControlledServosTorso");
            servos_to_test = {0, 1};
            ...
            servos_to_test.set_name("CurrentControlledServosFullBody");
        ...
        coefficientsPath = coefficientsPath + "/CurrentPositionMapping/models/coefficients.json";
            ...
            coeffcient_matrix = CreateCoefficientsMatrix(current_coefficients, servos_to_test, prediction_model.as_string(), servo_names);
    ...
    {   
        ...
        // === STATIC TEST MODE ===
        // Handle static testing with timed push prompts
        if (static_test_mode) {
        ...
        if (GetTick() <= 2) {
            ...
            for (int i = 0; i < servos_to_test.size(); i++) {
                starting_positions(transition, i) = present_position(servos_to_test(i));
            }
            
            // Update start position output if available from GoalSetter
            if (start_position_in.connected()) {
                starting_positions.copy(start_position_in);
            }
            return;
        }
        ...
        if (reached_goal.sum() == servos_to_test.size()) {
            ...
            if (transition < number_transitions) {
                ...
                matrix all_predicted = SetGoalCurrent(present_current, current_increment, current_limit,
                                                      present_position, goal_position_in, position_margin,
                                                      coeffcient_matrix, current_coefficients, std::string(prediction_model), model_prediction);
                for (int i = 0; i < servos_to_test.size(); i++) {
                    int servo_idx = servos_to_test(i);
                    predicted_current_history(transition, i) = all_predicted(servo_idx);
                }
            } else {
        ...
        if (GetTick() >= 2 && predicted_current_history.sum() == 0) {
            matrix all_predicted = SetGoalCurrent(present_current, current_increment, current_limit,
                                                  present_position, goal_position_in, position_margin,
                                                  coeffcient_matrix, current_coefficients, std::string(prediction_model), model_prediction);
    ...
    void SaveCurrentData()
    {
        try
        {
            ...
            for (int i = 0; i < servos_to_test.size(); i++)
            {
                file << "      \"" << servo_names[servos_to_test(i)] << "\": {\n";
                ...
                file << "        \"goal_positions\": [";
                for (int t = 0; t < transition; t++) {
                    file << goal_position_in(servos_to_test(i));
                    if (t < transition - 1) file << ", ";
                }
                file << "],\n";
                ...
                for (int t = 0; t < transition; t++) {
                    file << predicted_current_history(t, i);
                    if (t < transition - 1) file << ", ";
                }
                file << "],\n";
                ...
                file << "      }";
                if (i < servos_to_test.size() - 1) file << ",";
                file << "\n";
            }
...
void TestMapping::LogStaticPushData() {
    if (!static_test_mode || !static_push_log_file.is_open()) {
        return;
    }
