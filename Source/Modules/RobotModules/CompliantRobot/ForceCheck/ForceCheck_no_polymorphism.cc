...
class ForceCheck: public Module
{
    ...
    void Init()
    {
        ...
        number_deviations_per_time_window.set(0); // Initialize to 0 for each servo
    ...
    {   
        ...
        if (present_position.connected() && goal_position_in.connected() && !firstTick){ // current_prediction and present_current checked above
            ...
            if (!force_output_tapped){
                ...
                force_output.set(clip(force_output[0] + (float)force_change_rate, 0, 100));
                
                Debug("ForceCheck: Force output not tapped - setting force to " + std::to_string(force_output[0]));
            }
            ...
            if (deviance_ratio > 0.95 && !force_output_tapped){
                ...
                force_output_tapped_time_point = std::chrono::steady_clock::now();
                Debug("ForceCheck: Force output tapped triggered by high deviance");

            }

            // Reset force_output_tapped after timeout
            if (force_output_tapped && (std::chrono::steady_clock::now() - force_output_tapped_time_point > std::chrono::seconds(2))){
                force_output_tapped = false;
                
                Debug("ForceCheck: Force output tap timeout - resetting");
            }
        
            Debug("ForceCheck: Force output tapped time: " + std::to_string((std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - force_output_tapped_time_point).count())/1000 ) + " seconds");
        ...
        firstTick=false;
        Debug("ForceCheck: Number of deviations in time window: " + number_deviations_per_time_window.json());
    }
