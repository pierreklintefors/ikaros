...
// Helper struct for deviation statistics - optimized version
struct DeviationStats {
    ...
    int exceed_count = 0;
    int total_samples = 0;
    double recent_mean = 0.0; // Exponential moving average for smoothing
    double sum = 0.0;         // Cache sum for calculations
    double sum2 = 0.0;        // Cache sum of squares for calculations
    
    // Default constructor for cache initialization
    DeviationStats() = default;
    
    // Optimized constructor - only use for initial calculation
    DeviationStats(const matrix& deviation_history, int motor_idx, double threshold, int rows = -1) {
        ...
        // Single pass calculation with optimized loops
        double recent_sum = 0.0;
        int recent_count = std::max(1, actual_rows / 4);
        int recent_start = std::max(0, actual_rows - recent_count);
        ...
        for (int r = 0; r < actual_rows; ++r) {
            ...
            // Calculate recent_mean in same loop to avoid second pass
            if (r >= recent_start) {
                recent_sum += v;
            }
        }
        ...
        if (total_samples > 0) {
            ...
            recent_mean = (recent_count > 0) ? recent_sum / recent_count : mean;
    ...
    // Fast update for adding new value (for growing buffers)
    void addValue(double new_value, double threshold) {
        double abs_new = std::abs(new_value);
        
        sum += abs_new;
        sum2 += abs_new * abs_new;
        total_samples++;
        if (abs_new > threshold) exceed_count++;
        
        mean = sum / total_samples;
        exceed_ratio = (double)exceed_count / total_samples;
        
        // Update recent_mean using exponential moving average (fast and smooth)
        if (total_samples == 1) {
            recent_mean = abs_new;
        } else {
            recent_mean = 0.8 * recent_mean + 0.2 * abs_new;
        }
...
public:
    ...
    virtual void HandleDeviation(matrix &deviation, matrix &present_position,
                                 ...
                                 double current_time, double peak_height) = 0;
...
public:
    ...
    void HandleDeviation(matrix &deviation, matrix &present_position,
                         ...
                         matrix& motor_in_motion,
                         double pullback_amount, double peak_width_tolerance,
                         double current_time, double peak_height) override
    {
        ...
        if (rows > 0) {
            for (int i = 0; i < deviation.size(); i++) {
                ...
                if (stats.exceed_ratio > peak_width_tolerance && !obstacle_detected) {
                    an_obstacle_requires_active_avoidance_this_tick = true;
                    halt_goal_position.copy(present_position);
                    parent_component->Debug("NormalMode: Motor " + std::to_string(i) + 
                        " exceedance ratio " + std::to_string(stats.exceed_ratio) + 
                        " over history, initiating halt.");
                    break;
                }
            }
        }
        ...
        if (obstacle_detected && (current_time - last_obstacle_time > 2.0)) { 
        ...
        if (should_override) {
            ...
            parent_component->Trace("NormalMode: Override active, goal_position_out = " + goal_position_out.json());
        } else {
            goal_position_out.reset(); // Let GoalSetter handle normal operation
            parent_component->Trace("NormalMode: No override, goal_position_out after reset = " + goal_position_out.json());
        }
        parent_component->Trace("NormalMode: Present tilt position: " + std::to_string(present_position(0)));
    }
...
public:
    ...
    // External trigger used right after switching to this mode for immediate pullback
    void ExternalTriggerAvoidance(const matrix& deviation_in,
                                  const matrix& present_position_in,
                                  double pullback_amt,
                                  double now)
    {
        if (present_position_in.size() == 0 || goal_position_out.size() == 0)
            return;
        avoidance_goal_position.copy(present_position_in);
        for (int i = 0; i < deviation_in.size(); ++i) {
            double dir = (deviation_in(i) >= 0.0) ? 1.0 : -1.0;
            avoidance_goal_position(i) = present_position_in(i) + dir * pullback_amt;
        }
        obstacle_detected = true;
        last_avoidance_trigger_time = now;
        goal_position_out.copy(avoidance_goal_position);
        parent_component->Debug("AvoidantMode: ExternalTriggerAvoidance activated (immediate retract).");
    }
    void HandleDeviation(matrix& deviation, matrix& present_position,
                        matrix& goal_position_in, matrix& start_position,
                        matrix& allowed_deviance, matrix& started_transition,
        ...
        if (!obstacle_detected) {
            ...
            for (int i = 0; i < deviation.size(); i++) {
                ...
                if (has_previous)
                {
                    // Treat peaks as external if they oppose the commanded direction while moving
                    double cmd_delta = (double)goal_position_in(i) - (double)present_position(i);
                    int s_cmd = (cmd_delta > 0) - (cmd_delta < 0);
                    int s_dev = ((double)deviation(i) > 0) - ((double)deviation(i) < 0);
                    bool external_like = (motor_in_motion(i) == 1) ? (s_cmd == 0 || s_dev != s_cmd) : true;

                    // Additional temporal filter: Check if motor is in early movement phase
                    bool early_movement_phase = false;
                    if (motor_in_motion(i) == 1 && started_transition(i) != 0.0) {
                        // Use transition progress as temporal cue (early phase = more self-generated spikes)
                        double movement_progress = std::abs((double)present_position(i) - (double)start_position(i));
                        double total_movement = std::abs(cmd_delta) + movement_progress;
                        early_movement_phase = (total_movement > 0) && (movement_progress / total_movement < 0.3); // First 30% of movement
                    }

                    // Combine direction and temporal filters
                    bool likely_external = external_like && !early_movement_phase;

                    sharp_peak = (lastAbs > stats.mean + peak_height * stats.stddev) &&
                                 (prevAbs <= (double)allowed_deviance(i)) &&
                                 (stats.exceed_ratio <= peak_width_tolerance) &&
                                 ((motor_in_motion(i) == 1) ? (started_transition(i) != 0.0) : true) &&
                                 likely_external;
                }
...
public:
    ...
    void HandleDeviation(matrix& deviation, matrix& present_position,
        ...
        if (rows > 0) {
            for (int i = 0; i < deviation.size(); i++) {
                DeviationStats stats(deviation_history, i, allowed_deviance(i), rows);
                if (stats.mean > (double)allowed_deviance(i) || stats.exceed_ratio > 0.5) {
...
public:
    ...
    void SwitchMode(int new_mode) {
        ...
        parent_component->Warning("ModeController: Switching to mode index " + std::to_string(mode_index));
        switch (mode_index) {
        case 0:
            current_mode = std::make_unique<NormalMode>(parent_component, p_force_output, p_goal_position_out, p_led_intensity, p_led_color_eyes, p_led_color_mouth);
            break;
    ...
    void HandleDeviation(matrix &deviation, matrix &present_position,
                        ...
                        double current_time = 0.0 , double peak_height = 2.0)
    {
        if (!current_mode)
        {
        ...
        current_mode->HandleDeviation(deviation, present_position, goal_position_in,
                                     ...
                                     deviation_history, torque, motor_in_motion, pullback_amount, peak_width_tolerance, current_time, peak_height);
    }
    ...
    // Trigger avoidance immediately if currently in Avoidant mode (mode_index == 1)
    void TriggerImmediateAvoidance(const matrix& deviation,
                                   const matrix& present_position,
                                   double pullback_amount,
                                   double now)
    {
        if (mode_index != 1 || !current_mode) return;
        if (auto* av = dynamic_cast<AvoidantMode*>(current_mode.get())) {
            av->ExternalTriggerAvoidance(deviation, present_position, pullback_amount, now);
        }
    }
};
...
public: // Ensure INSTALL_CLASS can access constructor if it's implicitly used.
    ...
    matrix previous_current_prediction; // Store prediction from tick t to compare with current at tick t+1
    ...
    // Performance optimization: cached deviation statistics
    std::vector<DeviationStats> cached_motor_stats;
    matrix last_deviation_values; // Track last deviation for incremental updates
    bool stats_cache_initialized = false;
    ...
    // --- Helper methods (keep Tick readable) ---
private:
    // Motor is strictly stationary: no motion AND not in transition
    bool IsStationaryMotor(int i) const {
        if (i >= motor_in_motion.size() || i >= started_transition.size()) return false;
        return (motor_in_motion(i) == 0.0) && (started_transition(i) == 0.0);
    }

    // Detect a fast push matching the blue envelope; requires stationarity; uses PeakHeight/PeakWidthTolerance
    bool DetectFastPushStationaryThisTick() {
        const int rows = deviation_history.rows();
        if (rows <= 5 || deviation.size() == 0) return false;

        const double p_th = std::max(0.55, (double)peak_width);
        const double spike_width_tol = std::min(0.25, (double)peak_width);
        const double sustained_z = 0.5 * (double)peak_height;
        const double z_th_peak = std::max(2.0, (double)peak_height);

        double tol = (double)peak_width;
        if (tol < 0.05) tol = 0.05;
        if (tol > 0.50) tol = 0.50;
        const int win = std::max(10, std::min(rows, (int)std::round(rows * tol)));
        const int start = std::max(0, rows - win);

        for (int i = 0; i < deviation.size(); ++i) {
            if (!IsStationaryMotor(i)) continue; // Avoidant only when stationary

            // Stats on signed deviations in window
            double sum = 0.0, sum2 = 0.0, v_max = -1e12; int n = 0;
            for (int r = start; r < rows; ++r) {
                double v = (double)deviation_history(r, i);
                sum += v; sum2 += v*v; v_max = std::max(v_max, v); n++;
            }
            if (n < 3) continue;

            const double mu = sum / n;
            const double var = std::max(1e-6, (double)(sum2 / n) - mu * mu);
            const double sd  = std::sqrt(var);

            int above_cnt = 0;
            for (int r = start; r < rows; ++r) {
                double v = (double)deviation_history(r, i);
                if (v > (mu + sustained_z * sd)) above_cnt++;
            }
            const double p_pos = (double)above_cnt / n;
            const double z_peak = (v_max - mu) / (sd + 1e-6);

            const bool sustained_envelope = (p_pos >= p_th) && (z_peak >= z_th_peak);

            // Conservative sharp spike fallback using cached stats
            if (i < (int)cached_motor_stats.size()) {
                DeviationStats &stats = cached_motor_stats[i];
                const double lastAbs = std::abs((double)deviation(i));
                const double base_thr = stats.mean + 1.5 * stats.stddev; // stationary => smaller boost
                const bool sharp_peak = (lastAbs > (base_thr + 80.0)) && (stats.exceed_ratio <= spike_width_tol);
                if (sustained_envelope || sharp_peak) return true;
            } else if (sustained_envelope) {
                return true;
            }
        }
        return false;
    ...
    // Detect sustained hold/guidance; can occur mid-transition; uses cached stats and timers
    bool DetectSustainedHoldThisTick() {
        bool sustained_non_decreasing_deviation_this_tick = false;
        for (int i = 0; i < deviation.size(); i++) {
            if (i >= (int)cached_motor_stats.size()) continue;
            DeviationStats& stats = cached_motor_stats[i];
            bool sustained_force = stats.recent_mean > (double)allowed_deviance(i) * (double)sustained_force_threshold_ratio;
            if (sustained_force) {
                sustained_non_decreasing_deviation_this_tick = true;
                Debug("ForceCheck: Sustained force detected on motor " + std::to_string(i) +
                      ", recent_mean: " + std::to_string(stats.recent_mean) +
                      ", threshold: " + std::to_string((double)allowed_deviance(i) * (double)sustained_force_threshold_ratio));
                break;
            }
        }
        ...
        if (sustained_non_decreasing_deviation_this_tick) {
            if (!evaluating_sustained_hold) {
                ...
                double hold_duration = (GetTime() - sustained_high_dev_start_time) * 1000.0;
    ...
    // Calm/no obstacle conditions to return to Normal; safe to evaluate anytime
    bool ShouldReturnToNormalThisTick() {
        if (torque.sum() == 0) return false; // If torque disabled (compliant), don't return to normal
        ...
        for (int i = 0; i < deviation.size(); i++) {
            if (i >= (int)cached_motor_stats.size()) continue;
            DeviationStats& stats = cached_motor_stats[i];
            if (stats.recent_mean > (double)allowed_deviance(i)) significant_deviation_present = true;
            if (stats.exceed_ratio > (double)peak_width) obstacle_conditions_present = true;
        }
        return !(significant_deviation_present || obstacle_conditions_present);
    }
...
public:
    ...
    void Init()
    {
        ...
        Bind(peak_width, "PeakWidthTolerance"); // Width of the peak for sharp peak detection in percentage of deviance history
    ...
    void Tick()
    {
        ...
        // Compute deviation: prediction from tick t should match current at tick t+1
        // This aligns with how the model was trained (predicting next tick's current)
        ...
        // Use the prediction from the PREVIOUS tick (t) and compare with current at THIS tick (t+1)
        if (!firstTick && current_prediction.size() == present_current.size()) {
            deviation.copy(current_prediction);
            deviation.subtract(present_current);
        } else {
            // First tick: no previous prediction available, deviation will be zero
            if (deviation.size() != present_current.size()) {
                deviation.realloc(present_current.size());
            }
            deviation.reset(); // Set to zero on first tick
        }
        
        // Store current prediction for next tick's comparison
        previous_current_prediction.copy(current_prediction);

        // Simplified deviation history management with cached stats
        if (deviation.size() > 0) {
            // Initialize cache if needed
            if (!stats_cache_initialized || cached_motor_stats.size() != deviation.size()) {
                cached_motor_stats.resize(deviation.size()); // This is correct for std::vector
                last_deviation_values.realloc(deviation.size()); // Use realloc for matrix initialization
                last_deviation_values.set(0.0);
                stats_cache_initialized = true;
            }
            ...
            if (deviation_history.cols() != deviation.size()) {
                ...
                cached_motor_stats.clear();
                cached_motor_stats.resize(deviation.size()); // std::vector resize
                for (int i = 0; i < deviation.size(); i++) {
                    cached_motor_stats[i] = DeviationStats();
                }
            }
            
            // Simplified history management: just use realloc when buffer gets too large
            if (deviation_history.rows() >= history_lenght.as_int()) {
                // Reset the buffer instead of trying to erase rows (which doesn't exist)
                deviation_history.realloc(0, deviation.size()); // Use realloc for matrix
                // Reinitialize stats when buffer resets - ensure proper size
                cached_motor_stats.clear();
                cached_motor_stats.resize(deviation.size()); // std::vector resize
                for (int i = 0; i < deviation.size(); i++) {
                    cached_motor_stats[i] = DeviationStats();
            ...
            // Add new deviation values and update cached stats
            try {
                deviation_history.push(deviation, true);
                
                // Update cached stats incrementally for growing buffer
                for (int i = 0; i < deviation.size(); i++) {
                    // Ensure cached_motor_stats has enough elements
                    if (i < cached_motor_stats.size()) {
                        cached_motor_stats[i].addValue(
                            deviation(i), 
                            allowed_deviance.size() > i ? allowed_deviance(i) : 100.0
                        );
                    }
                }
                
                last_deviation_values.copy(deviation);
        ...
        {
            double base = (double)allowed_deviance_input(i);

            // Commanded direction from current goal vs present
            double cmd_delta = (double)goal_position_in(i) - (double)present_position(i);
            int s_cmd = (cmd_delta > 0) - (cmd_delta < 0);
            int s_dev = ((double)deviation(i) > 0) - ((double)deviation(i) < 0);
            bool same_dir = (s_cmd != 0) && (s_cmd == s_dev);

            bool moving = (motor_in_motion(i) == 1) && (started_transition(i) != 0.0);

            // Velocity-based scaling: use actual movement speed for natural attenuation
            double velocity_factor = 1.0;
            if (moving && same_dir) {
                // Calculate actual velocity (deg/tick)
                double actual_velocity = 0.0;
                try {
                    matrix prev_pos = present_position.last();
                    if (prev_pos.size() > i) {
                        actual_velocity = std::abs((double)present_position(i) - (double)prev_pos[i]);
                    }
                } catch (...) {}
                
                // Natural scaling: faster movement = more tolerance (capped at 3x)
                velocity_factor = 1.0 + std::min(0.5, actual_velocity / 2.0); // Scale by velocity/2, max 3x
                // Fallback to scalar if no velocity history
                if (actual_velocity == 0.0) {
                    velocity_factor = (double)movement_threshold_scalar;
                }
            }

            allowed_deviance(i) = base * velocity_factor;

            if (deviation(i) > allowed_deviance(i))
            {
                Debug("ForceCheck: Motor " + std::to_string(i) + " deviation: " + std::to_string(deviation[i]) +
                        ", exceedes threshold: " + std::to_string(allowed_deviance(i)) +
                        ", Motor moving: " + std::to_string(motor_in_motion[i]));
            }
            else if (deviation(i) > 400)
            {
                Debug("ForceCheck: Motor " + std::to_string(i) + " deviation: " + std::to_string(deviation[i]) +
                        ", allowed threshold: " + std::to_string(allowed_deviance(i)) +
                        ", Motor moving: " + std::to_string(motor_in_motion[i]));
            }
        }
            ...
            mode_controller->HandleDeviation(deviation, present_position, goal_position_in,
                                             start_position, allowed_deviance, started_transition,
                                             deviation_history, torque, motor_in_motion, (double)pullback_amount, (double) peak_width, GetTime(), (double) peak_height);
            ...
            if (automatic_mode_switching_enabled && !refractory_period && deviation_history.rows() > 5)
            {
                ...
                // Compute stats per motor over the last HistoryLength samples - use cached stats!
                int rows = deviation_history.rows();
                

                // 1. Detect Fast Push (Avoidant trigger) - only when stationary
                fast_push_detected_this_tick = DetectFastPushStationaryThisTick();

                // 2. Detect Sustained Force (Compliant trigger)
                being_held_detected_this_tick = DetectSustainedHoldThisTick();

                // 3. Assess calm conditions to return to Normal
                should_return_to_normal = ShouldReturnToNormalThisTick();
                ...
                if (desired_mode_idx != current_mode_idx)
                {
                    ...
                    // On Avoidant due to push, retract immediately
                    if (desired_mode_idx == 1 && fast_push_detected_this_tick) {
                        mode_controller->TriggerImmediateAvoidance(deviation, present_position,
        ...
        current_prediction.print(); // Debug print of current prediction
        present_current.print();    // Debug print of present current
        deviation.print();          // Debug print of deviation
